export default {
  //(Please Do Not Remove The comma(,) after every variable)
  //Change The Website Template

  //   Header Details ---------------------
  name: "Yamiao",
  headerTaglineOne: "Hello 👋",
  headerTaglineTwo:
    "I'm Yamiao Liu",

  socials: [
    { title: "Resume", link: "https://drive.google.com/file/d/1ienkLT6xw4ePzLytimlHfutMDrVqjbg0/view?usp=sharing" },
    { title: "Github", link: "https://github.com/ymliu978" },
    { title: "LinkedIn", link: "https://www.linkedin.com/in/yamiaoliu/" },
    { title: "Instagram", link: "https://www.instagram.com/ymliu978/" },
    { title: "Email", link: "mailto:ymliu97@gmail.com" },
  ],

  // End Header Details -----------------------

  // Work Section ------------------------
  projects: [
    // {
    //   title: "Peter Logan's Steak House", //Project Title - Add Your Project Title Here
    //   description: "CSS Flexbox and Grid, JavaScript, Adobe XD, Netlify",
    //   //Project Image - Add Your Project Image Here
    //   // imageSrc:"https://i.ibb.co/56BdKyd/peter-logan-landing-4x6.jpg",
    //   imageSrc:"images/peter-logan-landing-4x6.jpg",
    //   //Project URL - Add Your Project Url Here
    //   url: "https://peter-logans-steakhouse.netlify.app/",
    // },
    // {
    //   title: "React Travel Journal", //Project Title - Add Your Project Title Here
    //   description: "ReactJS, React Props, React Compinents, Figma, Vercel",
    //   //Project Image - Add Your Project Image Here
    //   imageSrc:
    //     "images/desktop-view.png",
    //   //Project URL - Add Your Project Url Here
    //   url: "https://travel-journal-react-zeta.vercel.app/",
    // },
    // {
    //   title: "The Invisible Guardian", //Project Title - Add Your Project Title Here
    //   description: "As a game designer, Tencent",
    //   //Project Image - Add Your Project Image Here
    //   imageSrc:"images/Invisible-Guardian-min.png",
    //   //Project URL - Add Your Project Url Here
    //   url: "https://store.steampowered.com/app/998940/_The_Invisible_Guardian/?cc=cn",
    // },
    /*
    If You Want To Add More Project just Copy and Paste This At The End
    ,{
        title: 'Project Five',
        description: 'Something Amazing',
        imageSrc: "",
        url: ''
    }
    */
  ],

  // End Work Section -----------------------

 // Game Section ------------------------
 projects2: [
  {
    title: "uOttaHack 5 Winner", //Project Title - Add Your Project Title Here
    description: "React, OCR, APIs, Sponsor Chanllenge 🥉",
    //Project Image - Add Your Project Image Here
    // imageSrc:"https://i.ibb.co/56BdKyd/peter-logan-landing-4x6.jpg",
    imageSrc:"images/uottahack5.png",
    //Project URL - Add Your Project Url Here
    url: "https://devpost.com/software/care-scan",
  },
  // {
  //   title: "React Travel Journal", //Project Title - Add Your Project Title Here
  //   description: "ReactJS, React Props, React Compinents, Figma, Vercel",
  //   //Project Image - Add Your Project Image Here
  //   imageSrc:
  //     "images/desktop-view.png",
  //   //Project URL - Add Your Project Url Here
  //   url: "https://travel-journal-react-zeta.vercel.app/",
  // },
  {
    title: "The Invisible Guardian", //Project Title - Add Your Project Title Here
    description: "As a game designer, Tencent",
    //Project Image - Add Your Project Image Here
    imageSrc:"images/Invisible-Guardian-169.jpg",
    //Project URL - Add Your Project Url Here
    url: "https://store.steampowered.com/app/998940/_The_Invisible_Guardian/?cc=cn",
  },
  {
    title: "The Master", //Project Title - Add Your Project Title Here
    description: "Unity, Physics Game",
    //Project Image - Add Your Project Image Here
    imageSrc:"https://img.itch.zone/aW1hZ2UvNjQ0ODAzLzM0NTI3NjYuZ2lm/original/7M2J%2Bj.gif",
    //Project URL - Add Your Project Url Here
    url: "https://houkanshan.itch.io/the-master",
  },
  {
    title: "Cat Walking Sim", //Project Title - Add Your Project Title Here
    description: "Unity, Puzzle Game",
    //Project Image - Add Your Project Image Here
    imageSrc:"https://img.itch.zone/aW1hZ2UvNTM2NzcwLzI3OTI2OTguZ2lm/original/ecoGk5.gif",
    //Project URL - Add Your Project Url Here
    url: "https://houkanshan.itch.io/cat-walker-simulator",
  },
  // {
  //   title: "Unmanned Station", //Project Title - Add Your Project Title Here
  //   description: "Unity, Sim Game",
  //   //Project Image - Add Your Project Image Here
  //   imageSrc:"https://i.ibb.co/88wy5D5/Unmanned-Station169.gif",
  //   //Project URL - Add Your Project Url Here
  //   url: "https://ymliu.itch.io/unmanedstation",
  // },
  {
    title: "I love you, but", //Project Title - Add Your Project Title Here
    description: "Board Game",
    //Project Image - Add Your Project Image Here
    imageSrc:"images/ILoveYouBut169.gif",
    //Project URL - Add Your Project Url Here
    url: "https://iloveyoubut.cargo.site/About",
  },
  // {
  //   title: "Even I Can Hear", //Project Title - Add Your Project Title Here
  //   description: "Blender, 3D Modelling",
  //   //Project Image - Add Your Project Image Here
  //   imageSrc:"https://i.ibb.co/Kb0BqgH/Even-ICan-Hear.gif",
  //   //Project URL - Add Your Project Url Here
  //   url: "https://iloveyoubut.cargo.site/About",
  // },
  {
    title: "Peter Logan's Steak House", //Project Title - Add Your Project Title Here
    description: "Web Dev, Dark Mode, Multi Languages, Adobe XD",
    //Project Image - Add Your Project Image Here
    // imageSrc:"https://i.ibb.co/56BdKyd/peter-logan-landing-4x6.jpg",
    imageSrc:"images/peter-logan-landing-4x6.jpg",
    //Project URL - Add Your Project Url Here
    url: "https://peter-logans-steakhouse.netlify.app/",
  },
  {
    title: "A Donut", //Project Title - Add Your Project Title Here
    description: "Blender, 3D Modelling",
    //Project Image - Add Your Project Image Here
    imageSrc:"images/DonutTheBest-min.png",
    //Project URL - Add Your Project Url Here
    url: "https://ibb.co/SXy9m2k",
  },
 
],

// End Game Section -----------------------

  // Services Section ---------------

  services: [
    // {
    //   title: "Web Design and Development",
    //   description:
    //     "I have a keen eye for aesthetics and the ability to design websites from scratch, then translate the visual design into adaptable code.",
    // },
    {
      title: "Mutlimedia Design and Development",
      description:
        "I have a keen eye for aesthetics and the ability to design and develop games, websites, and other digital products.",
    },
    {
      title: "Communication and Problem-solving",
      description:
        "I know how to effectively communicate with clients via email and phone to bring their vision to life. I am self-directed and adopt new processes quickly.",
    },
    {
      title: "Tools",
      description:
        "Unity Engine, Blender, Visual Studio, Git, MySQL Workbench, Adobe InDesign, Adobe XD, Adobe Premiere Pro, and more...",
    },
    {
      title: "Technologies",
      description:
        "HTML5, CSS3, JavaScript, ReactJS, jQuery, C#, SQL, Python, PHP, and more...",
    },
  ],

  // End Services Section --------------------------

  // About Secton --------------
  aboutparaone:
    "I am a creator that specializes in designing and developing interactive digital experiences. I love bridging the gap between art and programming.",
  aboutparatwo:
    "Seriously, they make each other better.",
  //   End About Section ---------------------
};

// Thanks for using this template, I would love to hear from you contact me at hello@chetanverma.com
